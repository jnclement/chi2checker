  ...
  for(int i=0; i<nums1.size(); ++i)
    {
      ...
      nums1.at(i)->GetXaxis()->SetRangeUser(30,100);
...
int draw_spec()
{
  ...
  TH3D* h3_pt_lem_loh = (TH3D*)inf->Get("h3_pt_tpt_loh");
  TH3D* h3_tpt_lem_loh = (TH3D*)inf->Get("h3_tpt_lem_loh");

  std::vector<vector<int>> ybounds = {{61,70}};
  std::vector<vector<int>> zbounds = {{1,120}};
  std::vector<int> axis = {1};
  std::vector<int> colors = {kSpring};//, kAzure};//, kViolet, kOrange, kGray};
  std::vector<int> markers = {20};//, 21};//, 71, 72, 88};
  std::vector<string> numlabels = {"Truth jets"};//"EM fraction < 0.9 only","EM fraction > 0.1 only","OH fraction < 0.9 only","OH Fraction > 0.1 only","EM frac <0.9 && OH frac > 0.1"};

  drawprettyeff(h3_pt_lem_loh,ybounds,zbounds,axis,colors,markers,numlabels,"h3_pt_lem_loh_tspec.png");
