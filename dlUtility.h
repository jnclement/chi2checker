...
void zcut_text(int zcut, float xp = 0.7, float yp =0.8, bool isRightAlign=0, double textsize = 0.04)
{
  string text = "|z_{vtx}|<"+to_string(zcut)+" cm";
  if(zcut == 0) text = "No z_{vtx} cut";
  drawText(text.c_str(),xp,yp,isRightAlign,kBlack,textsize);
}
...
{
  drawText("#bf{#it{sPHENIX}} Sim", xpos, ypos, ra, kBlack, textsize);
}
