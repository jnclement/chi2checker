...
int drawtiming(int lobinx, int hibinx, int lobiny, int hibiny, int lobinz, int hibinz, int isdat = 1)
{
  ...
  for(int i=0; i<ncut; ++i)
    {
      ...
      h3_pt_dt_t[i][0]->GetXaxis()->SetRange(0,100);
  ...
  TH1D* h_ratio_frac = (TH1D*)h3_projx[2][0][0]->Clone((string(h3_projx[2][0][0]->GetName())+"clone").c_str());
  h_ratio_frac->Divide(h3_projx[2][0][0],h3_projx[3][0][0],1,1,"B");
  ...
  c->SaveAs((typestr+"_ratio_frac.png").c_str());
  ...
  vector<string> towrite = {"All jets with E_{T} > 30 GeV in events passing dijet or fraction cut OR with a leading E_{T}^{jet} > 50 GeV","All jets with E_{T} > 30 GeV in events passing dijet cut"};
  ...
  //h_spectrum_dijetcut->GetXaxis()->SetRangeUser(50,100);
  h_spectrum_dijetcut->Draw("PE");
  h_spectrum_tdijetcut->Draw("SAME PE");
  leg->Draw();
  d->cd(2);
  //h_ratio_dijetcut->GetXaxis()->SetRangeUser(50,100);
  ...
  //maxval = 2*max(h_spectrum_tall->GetMaximum(),h_spectrum_all->GetMaximum());
  //minval = 2*min(h_spectrum_tall->GetMinimum(),h_spectrum_all->GetMinimum());
  h_spectrum_all->GetYaxis()->SetRangeUser(minval,maxval);
  //h_spectrum_all->GetXaxis()->SetRangeUser(50,100);
  h_spectrum_all->Draw("PE");
  h_spectrum_tall->Draw("SAME PE");
  leg->Draw();
  d->cd(2);
  //h_ratio_all->GetXaxis()->SetRangeUser(50,100);
  h_ratio_all->Draw("PE");
  d->cd(0);
  sphenixtext(0.7,0.98,0,0.02);
  sqrt_s_text(0.7,0.96,0,0.02);
